...
########################
#
# aegisub_convert_docs.pl - downloads and converts the SubStation documentation wiki to static HTML.
#
...
# variables
my $base_dir = '.';
my $host = 'http://docs.substation.org';
my $docs_base_url = 'http://docs.substation.org/manual/';
...
sub parse_and_fix_html {
	...
	foreach my $tag ( $html->links() ) {
		...
		if ( ($tagname eq 'a') and exists($attrs{'href'}) ) {
			...
			# does it go within substation.cellosoft.com?
			if ( $attrs{'href'} =~ m!^$quoted!i or (substr($attrs{'href'},0,1) eq '/') ) {
				push(@links_to_modify, $attrs{'href'});
			}
			# is not relative and goes somewhere else than substation.cellosoft.com
	...
	$url =~ s!http://docs.substation.org!!;
...
sub convert_link {
	...
	$link =~ s!http://docs.substation.org!!;
...
sub parse_css {
	...
	while ( $content =~ m!url\((\")?(.+?)(\")?\)!mgi ) {
		...
		# skip it if it's nonrelative and goes somewhere else than substation.cellosoft.com
		if ( $text =~ m!^http!i ) {
